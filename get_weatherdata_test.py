...
def test_coords_dim():
    ...
    assert len(
        json.loads(get_weather_data('Riga', api_key=key)).get('coord')) == 2, \
...
def test_temp_type():
    ...
    assert type(json.loads(get_weather_data('Riga', api_key=key)).get(
        'feels_like')) is float, \
...
def test_countries(city, api_key, expected_country):
    ...
    assert json.loads(get_weather_data(city, api_key=key)).get('country',
...
def test_utc_time(city, api_key, expected_time):
    import json
    assert json.loads(get_weather_data(city, api_key=key)).get('timezone',
